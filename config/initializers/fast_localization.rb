ENV['FAST_LOC'] ||= '1' if Rails.env.development?
...
if ENV['FAST_LOC'] == '1'
  ...
  # class_eval is necessary because remove_method is a private method
  ...
  Time Date Integer Float Rational Complex TrueClass FalseClass NilClass).each do |x|
    x.constantize.class_eval('remove_method :to_yaml if instance_methods(false).include?(:to_yaml)')
  end
end
